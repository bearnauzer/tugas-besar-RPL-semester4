...
if (isset($_GET['confirm']) && $_GET['confirm'] == 'yes') {
    $query_delete = "DELETE FROM notes_aroma_custom WHERE id = $id";
    
    if (mysqli_query($conn, $query_delete)) {
        echo "<script>alert('Notes aroma berhasil dihapus!'); window.location.href='produk.php';</script>";
    } else {
...
$query_nama = mysqli_query($conn, "SELECT nama FROM notes_aroma_custom WHERE id = $id");
...
<head>
    ...
    <title>Konfirmasi Hapus Notes - PERSCENTS</title>
    <style>
...
<body>
    ...
    <div class="card">
        ...
        <p>Apakah kamu yakin ingin menghapus bahan/notes <br> <span class="highlight-name">"<?= htmlspecialchars($data['nama']); ?>"</span>?<br>Data yang dihapus tidak dapat dikembalikan.</p>
        
        <div class="btn-group">
            <a href="javascript:history.back()" class="btn btn-batal">Batal</a>
            <a href="hapus_notes.php?id=<?= $id; ?>&confirm=yes" class="btn btn-yakin">Yakin, Hapus</a>
        </div>
    </div>
