...
$query = mysqli_query($conn, "SELECT * FROM mst_ukuran WHERE id = $id");
...
if (isset($_POST['submit'])) {
    ...
    $query_update = "UPDATE mst_ukuran SET nama = '$nama', ml = '$ml', tambahan_harga = '$tambahan_harga' WHERE id = $id";
    if (mysqli_query($conn, $query_update)) { echo "<script>alert('Data ukuran botol berhasil diperbarui!'); window.location.href = 'variant_management.php';</script>"; } 
...
<head>
    ...
    <title>Edit Ukuran Botol - PERSCENTS</title>
...
<body>
    ...
    <main class="main-wrapper">
        <header class="topbar">
            <h1 class="page-title">Edit Ukuran Botol</h1>
...
<div class="form-group"><label>Nama Ukuran (Contoh: Medium / Large)</label><input type="text" name="nama" class="form-control" value="<?= htmlspecialchars($data['nama']); ?>" required></div>
<div class="form-group"><label>Volume Botol (ml)</label><input type="number" name="ml" class="form-control" value="<?= $data['ml']; ?>" required></div>
