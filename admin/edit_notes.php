...
$query = mysqli_query($conn, "SELECT * FROM notes_aroma_custom WHERE id = $id");
...
if (isset($_POST['submit'])) {
    ...
    $query_update = "UPDATE notes_aroma_custom SET nama = '$nama', deskripsi = '$deskripsi', kategori = '$kategori', harga = '$harga' WHERE id = $id";
    if (mysqli_query($conn, $query_update)) { echo "<script>alert('Data notes berhasil diperbarui!'); window.location.href = 'produk.php';</script>"; } 
...
<head>
    ...
    <title>Edit Notes Aroma (Bahan) - PERSCENTS</title>
...
<body>
    ...
    <main class="main-wrapper">
        <header class="topbar">
            <h1 class="page-title">Edit Notes Aroma (Bahan)</h1>
...
<div class="form-group"><label>Nama Notes Aroma</label><input type="text" name="nama" class="form-control" value="<?= htmlspecialchars($data['nama']); ?>" required></div>
<div class="form-group"><label>Deskripsi Notes</label><textarea name="deskripsi" class="form-control"><?= htmlspecialchars($data['deskripsi']); ?></textarea></div>
<div class="grid-2" style="margin-bottom:0;">
    <div class="form-group"><label>Kategori (Gender)</label><select name="kategori" class="form-control" required>
        <option value="unisex" <?= ($data['kategori'] == 'unisex') ? 'selected' : ''; ?>>Unisex</option>
        <option value="pria" <?= ($data['kategori'] == 'pria') ? 'selected' : ''; ?>>Pria</option>
        <option value="wanita" <?= ($data['kategori'] == 'wanita') ? 'selected' : ''; ?>>Wanita</option>
    </select></div>
    <div class="form-group"><label>Harga Tambahan Custom (Rp)</label><input type="number" name="harga" class="form-control" value="<?= $data['harga']; ?>" required></div>
</div>
