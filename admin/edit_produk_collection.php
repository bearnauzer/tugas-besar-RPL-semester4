...
$query = mysqli_query($conn, "SELECT * FROM produk_collection WHERE id = $id");
$data = mysqli_fetch_assoc($query);

if (!$data) { echo "<script>alert('Data tidak ditemukan!'); window.location.href='produk.php';</script>"; exit; }
...
if (isset($_POST['submit'])) {
    ...
    $path_db_baru = !empty($data['foto']) ? $data['foto'] : $data['gambar']; 
    ...
    if (isset($_FILES['gambar']) && $_FILES['gambar']['error'] === 0) {
        ...
        if (in_array($ekstensi_file, $ekstensi_valid)) {
            ...
            if (move_uploaded_file($tmp_name, $lokasi_simpan)) {
                $path_db_baru = 'assets/uploads/' . $nama_file_baru;
            }
        } else { echo "<script>alert('Format foto harus JPG/PNG!');</script>"; }
    }

    $query_update = "UPDATE produk_collection SET nama = '$nama', deskripsi = '$deskripsi', harga_dasar = '$harga_dasar', kategori = '$kategori', tipe = '$tipe', gambar = '$path_db_baru', foto = '$path_db_baru', stok = '$stok' WHERE id = $id";
    if (mysqli_query($conn, $query_update)) { echo "<script>alert('Data berhasil diperbarui!'); window.location.href = 'produk.php';</script>"; } 
...
<head>
    ...
    <title>Edit Produk Collection - PERSCENTS</title>
...
<body>
    ...
    <main class="main-wrapper">
        <header class="topbar">
            <h1 class="page-title">Edit Produk Collection</h1>
            <a href="produk.php" class="btn-primary" style="background:#E9EDF7; color:#5A738E;"><i class="fa-solid fa-arrow-left"></i> Kembali</a>
        </header>
        <div class="form-card">
            <form action="" method="POST" enctype="multipart/form-data">
                
<div class="form-group"><label>Nama Produk</label><input type="text" name="nama" class="form-control" value="<?= htmlspecialchars($data['nama']); ?>" required></div>
<div class="form-group"><label>Deskripsi Produk</label><textarea name="deskripsi" class="form-control"><?= htmlspecialchars($data['deskripsi']); ?></textarea></div>
<div class="grid-2" style="margin-bottom:0;">
    <div class="form-group"><label>Kategori (Gender)</label><select name="kategori" class="form-control" required>
    ...
    <div class="form-group"><label>Tipe Katalog</label><select name="tipe" class="form-control" required>
        <option value="mbti" <?= ($data['tipe'] == 'mbti') ? 'selected' : ''; ?>>MBTI</option>
        <option value="karakter" <?= ($data['tipe'] == 'karakter') ? 'selected' : ''; ?>>Karakter Kucing</option>
    </select></div>
</div>
<div class="grid-2" style="margin-bottom:0;">
    <div class="form-group"><label>Harga Dasar (Rp)</label><input type="number" name="harga_dasar" class="form-control" value="<?= $data['harga_dasar']; ?>" required></div>
    <div class="form-group"><label>Stok Tersedia</label><input type="number" name="stok" class="form-control" value="<?= $data['stok']; ?>" required></div>
</div>
<div class="form-group"><label>Ganti Foto Produk (Opsional)</label><input type="file" name="gambar" class="form-control" accept="image/png, image/jpeg, image/jpg"><span style="font-size: 12px; color: #A3AED0; margin-top: 8px; display: block;"><i class="fa-solid fa-circle-info"></i> Kosongkan jika tidak ingin mengganti foto saat ini.</span></div>
